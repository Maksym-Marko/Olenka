...
const TEMPLATE = [
	['olenka/badge', {
		text: 'WORDPRESS BLOCK THEME STARTER KIT',
		squareBackgroundColor: '#a95f33',
		textColor: '#c97a45',
		className: 'text-base',
	}],
	['core/heading', {
		content: 'A clean foundation for building WordPress block themes',
		level: 2,
		style: {
			typography: {
				lineHeight: '1.2',
	...
	['core/paragraph', {
		content: 'Olenka is a developer-focused boilerplate for building modern WordPress block themes. It gives you a structured starting point with Tailwind CSS, Vite-powered assets, Composer autoloading, custom Gutenberg blocks, reusable templates, template parts, and block patterns.',
		style: {
			color: { text: '#7a675c' },
			elements: { link: { color: { text: '#7a675c' } } },
			spacing: { margin: { top: 'var:preset|spacing|mini' } },
		},
		fontSize: 'xx-small',
	}],
	['core/buttons', {
		style: { spacing: { margin: { top: 'var:preset|spacing|small' } } },
	}, [
		['core/button', {
	...
	['core/paragraph', {
		content: 'Built for developers who want a clear starting point instead of a heavy visual theme.',
		style: {
			color: { text: '#7a675c' },
			elements: { link: { color: { text: '#7a675c' } } },
...
export default function edit({ attributes, setAttributes }) {
	const { imageUrl, imageId, imageAlt, imageBackgroundOpacity } = attributes;
	...
	return (
		<>
			...
			<div {...blockProps}>
				<div className="py-24 md:py-32 border-b border-coffee-02">
					<div className="max-w-5xl mx-auto px-6">
						<div className="flex flex-col md:flex-row md:items-start gap-12 md:gap-20">
							...
							<div
								className="bg-coffee-01 aspect-[480/640] w-[280px] flex flex-col items-center justify-center gap-2 flex-shrink-0 pt-2 rounded-md overflow-hidden"
								style={{ opacity: imageUrl ? imageBackgroundOpacity : 0.8 }}
							>
								{imageUrl ? (
									<img
										src={imageUrl}
									...
									<>
										...
										<p className="text-base text-coffee-03">
											{__('Add image from sidebar', metadata.textdomain)}
										</p>
									</>
								)}
							</div>
